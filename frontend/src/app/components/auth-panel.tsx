...
export function AuthPanel({ open, mode, onClose, onModeChange }: Props) {
  ...
  const onGoogleSignIn = () => {
    ...
    if (!clientId) {
      setFormError('Google sign-in is not configured. Add NEXT_PUBLIC_GOOGLE_CLIENT_ID to .env.local (see .env.example), then restart the dev server.')
      return
    }
              ...
              {mode === 'login' && (
                <div className="mt-3">
                  <button
                    ...
                    className="w-full inline-flex items-center justify-center gap-2 rounded-xl border border-neutral-200 px-4 py-3 text-sm bg-white hover:bg-neutral-50"
                  >
